...
import { Award, BookOpen, Star, Scale, Shield, Users, Briefcase, ArrowRight, Check, MapPin, FileText } from 'lucide-react';
...
const CREDENTIALS = [
  ...
  { icon: BookOpen, title: 'Education',      desc: 'Juris Doctor, Philippines' },
  { icon: MapPin,   title: 'IBP Chapter',    desc: 'Negros Oriental' },
  { icon: FileText, title: 'Practice',       desc: 'Civil, Family & Corporate' },
];
    ...
    <div className="grid grid-cols-2 gap-3" style={{ height: '260px', gridTemplateRows: '1fr 1fr' }}>
      {items.map(({ icon: Icon, title, desc }) => (
        <div key={title} className="bg-gradient-to-br from-white/15 to-white/5 border border-[#D4AF37]/30 rounded-xl p-5 hover:border-[#D4AF37]/60 hover:bg-white/20 transition-all flex flex-col justify-center">
          ...
          <p className="text-sm font-bold text-white leading-tight tracking-wide">{title}</p>
...
export default function HeroSection() {
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(delta) > 40) {
      ...
      const prev = (slideRef.current + SLIDES.length - 1) % SLIDES.length;
      ...
      <div ref={ref} className="container mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center pt-24 lg:pt-28 pb-12">
        ...
        <div className="order-2 lg:order-1">
          <div className={`flex items-center gap-2 mb-6 pre-animate ${isInView ? 'animate-fade-in-up' : ''}`}>
            <Scale size={14} style={{ color: '#A67C00' }} />
            <span style={{ color: '#A67C00' }} className="font-medium">Boutique Solo Practice. Trusted Legal Solutions.</span>
          </div>
          ...
          <div className={`flex flex-wrap items-center gap-3 pre-animate ${isInView ? 'animate-fade-in-up delay-300' : ''}`}>
            <button
              ...
              className="inline-flex items-center gap-2 px-7 py-3.5 rounded-lg font-semibold text-sm transition-all bg-[#D4AF37] text-[#1A2B3C] hover:bg-[#BF9B2F] active:scale-95"
            >
              Contact Us
            </button>
            <a
              href="#services"
              className="inline-flex items-center gap-2 px-7 py-3.5 rounded-lg font-semibold text-sm border-2 transition-all text-[#1A2B3C] hover:bg-[#1A2B3C] hover:text-white active:scale-95"
              style={{ borderColor: '#1A2B3C' }}
            >
          ...
          <div className={`mt-10 grid grid-cols-3 gap-6 pre-animate ${isInView ? 'animate-fade-in-up delay-400' : ''}`}>
            <div className="text-center border-r border-[#E5E7EB]">
              <p className="text-3xl font-extrabold text-[#1A2B3C]" style={{ fontFamily: 'var(--font-playfair)' }}>IBP</p>
              <p className="text-xs text-[#6B7280] mt-1 font-medium">Member</p>
            </div>
            <div className="text-center border-r border-[#E5E7EB] flex flex-col items-center">
              <div className="flex items-center gap-1">
                <p className="text-3xl font-extrabold text-[#1A2B3C]" style={{ fontFamily: 'var(--font-playfair)' }}>5.0</p>
                <Star size={18} fill="#D4AF37" style={{ color: '#D4AF37' }} className="mb-0.5" />
              </div>
              <p className="text-xs text-[#6B7280] mt-1 font-medium">Google Rating</p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-extrabold text-[#1A2B3C]" style={{ fontFamily: 'var(--font-playfair)' }}>6+</p>
              <p className="text-xs text-[#6B7280] mt-1 font-medium">Practice Areas</p>
            </div>
          </div>
          <div className={`mt-6 h-px bg-[#E5E7EB] pre-animate ${isInView ? 'animate-fade-in-up delay-500' : ''}`} />
          <p className={`mt-3 text-xs text-[#94A3B8] flex items-center gap-1.5 pre-animate ${isInView ? 'animate-fade-in-up delay-500' : ''}`}>
            <Check size={12} className="text-[#A67C00] flex-shrink-0" />
        ...
        <div
          className={`order-1 lg:order-2 relative bg-[#1A2B3C] rounded-2xl p-6 lg:p-10 text-white border border-[#D4AF37]/20 shadow-2xl pre-animate ${isInView ? 'animate-fade-in-right delay-100' : ''}`}
        ...
        >
          ...
          <div className="flex items-center justify-between mb-8 pb-6 border-b border-white/10">
            <div>
              ...
              <p className="text-sm font-semibold" style={{ color: '#E8C547' }}>
                Solo Practitioner
              </p>
            </div>

            {/* Brand logo */}
            <div className="flex items-center gap-2 flex-shrink-0 border border-[#D4AF37]/30 rounded-xl px-3 py-2">
              <Scale size={18} style={{ color: '#D4AF37' }} />
              <span className="text-sm font-semibold text-white whitespace-nowrap" style={{ fontFamily: 'var(--font-playfair)' }}>
                Miller Law Office
              </span>
            </div>
          </div>
          ...
          >
            ...
            {currentSlide.type === 'photo' && currentSlide.photoSrc && (
              <div className="relative w-full rounded-xl overflow-hidden" style={{ height: '260px' }}>
                {failedPhotos.has(currentSlide.photoSrc) ? (
                  <div className="w-full h-full bg-gradient-to-br from-[#1A2B3C] to-[#243650] flex items-center justify-center">
                    <p className="text-[#D4AF37] font-bold text-lg text-center px-4" style={{ fontFamily: 'var(--font-playfair)' }}>
                  ...
                  <>
                    <Image
                      ...
                      fill
                      draggable={false}
                      onError={() => setFailedPhotos(prev => new Set([...prev, currentSlide.photoSrc!]))}
          ...
          {/* Dot nav */}
          <div className="flex items-center justify-center mt-6 pt-4 border-t border-white/10 gap-0.5">
            {SLIDES.map((s, i) => (
              <button
                type="button"
                key={s.label}
